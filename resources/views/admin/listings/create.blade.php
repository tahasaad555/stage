...
<div class="space-y-6">
    ...
    <div class="flex items-center">
        ...
        <div>
            ...
            <p class="text-gray-600">Create a new advertisement for agricultural land</p>
        </div>
    </div>
                ...
                <div>
                    ...
                    @if($lands->isEmpty())
                        <p class="mt-1 text-sm text-yellow-600">
                            ...
                            <a href="{{ route('admin.lands.create') }}" class="text-green-600 hover:text-green-800">Create a new land first</a>.
                        </p>
                    @endif
                </div>
            </div>
...
document.getElementById('terre_agricole_id').addEventListener('change', function() {
    ...
    if (selectedOption.value && !titleInput.value) {
        ...
        const landTitle = landText.split(' - ')[0];
        const region = landText.split(' - ')[1];
        
        if (landTitle && region) {
            titleInput.value = `${landTitle} - Available for Sale in ${region}`;
        }
    }
});
</script>
